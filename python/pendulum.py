...
canvas = Canvas(root, width=600, height=600,
                bg="grey29", bd=0, highlightthickness=0)
# binds an event handler to the canvas widget, calls mouseDown()
canvas.bind("<ButtonPress-1>", mouseDown)
canvas.bind("<ButtonRelease-1>", mouseUp)
# event is called if mouse is moved while left button is down
canvas.bind("<B1-Motion>", mouseDownMotion)
...
def drawLine(x1, y1, x2, y2):
    canvas.create_line(x1, canvas.winfo_height()-y1, x2,
                       canvas.winfo_height()-y2, width=lineWidth)


def drawCircle(x, y, r, c):
    canvas.create_oval(x - r, canvas.winfo_height()-y - r,
                       x+r, canvas.winfo_height()-y+r, fill=c, width=lineWidth)
...
def drawObjects():
    ...
    if(leftDown):
        ...
        endX = mouseX
        endY = mouseY
        length = math.sqrt(math.pow(pinY - mouseY, 2) +
                           math.pow(pinX - mouseX, 2))
    else:
