...
points = [Point(i, (height/2 * math.e**(-((i*pointWidth - width/2)/(width*0.1))**2)) + (-height/5 * math.e**(-((pointWidth*i - width/2)/(0.2*width))**2)) ) for i in range(pointCount)]
...
def simulateString():
    ...
    for i in range(1, pointCount -1):
        ...
        points[i].yA = (stringConstant/(pointWidth**2)) * (rightY - 2*y + leftY)
